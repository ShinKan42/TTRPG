...
使用者未变身时，对使用者`擒抱`的`检定`有`劣势`，且 `DC` = 20
...
- 选择以下其中一种效果发动：
    ...
    - 选取 1 个人类/`假面骑士`/`怪人`，进行`智力`/`感知`/`魅力`最高项的`豁免`，否则被震慑 1 `回合`
...
- 选择以下其中一种效果发动：
    ...
    - 选取 2 个人类/`假面骑士`/`怪人`，进行`智力`/`感知`/`魅力`最高项的`豁免`，否则被震慑 1 `回合`
...
- 被动效果：
    - 使用流水进行武器攻击时，可使用同样的`攻击检定`，更改为视为使用 2 环的“冰刃”：本体 1d10 `穿刺`，不论命中与否，目标及 5 ft 范围内每个生物进行`敏捷豁免`，失败受到 3d6 冷冻伤害
