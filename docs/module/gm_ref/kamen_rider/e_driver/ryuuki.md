...
- `攻击检定` +2
- 攻击伤害 +1d12
- 若攻击伤害的 d12 投出同等数值，则弹跳一次发动`攻击检定`，不可取同一目标，可重复弹跳
...
- `触及范围`： 15 ft
- 攻击命中时选择一种额外效果：
    - 无法再作出攻击，但对方须通过`敏捷豁免`结束`束缚`状态，否则每`回合`可使用`🟢标准动作`直接结算`攻击动作`命中并附加额外一次攻击伤害，`豁免`可于对方`回合`重复尝试
...
集群战术：若羊型镜怪人的至少一名盟友位于目标生物 5 ft 范围内且未处于失能状态，则该羊型镜怪人对该生物的`攻击检定`具有`优势`
...
- `🔺附赠动作`
- 6 点`逼格`
- 获得“加速术”正面效果，其中，额外动作可用于正常的多次攻击/使用魔法
