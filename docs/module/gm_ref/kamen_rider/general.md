...
- 警戒（可与自愿的他人互换先攻）
- 健壮
- 巫师杀手（伤害他人断`专注`劣势；智力、感知或魅力豁免失败时，可以将其改为成功，短休恢复）
...
此处的击败为：==最后一击== 或 ==最后一击时存在需要`专注`的持续性法术效果==
...
==场上累计==每击败 5 名`邪魔徒`/`杂鱼`级人物 或 2 名`假面骑士`时触发召唤（请注意，不包含`干部`/`怪兽`）

---

触发者`核心 ID` 对应/无`带扣`时可自由选择的 1 张`基础带扣` 或 根据随机表决定的 1 张`增强带扣`，且==不与场上存在的重复==
...
擒抱成功者，放弃擒抱效果，可改为==获得对方目前激活的所有`卡带`== 或 ==至多 2 张`卡带`==；若对方为骑士，需要==吃瘪点数条件满足==，成功后额外==强制结束`变身`==
